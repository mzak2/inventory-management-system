...
// make the add and remove buttons work
 ...
 //        return "confirmationassocpart";
    ...
    /*adds a method for calling the "buyProduct" method from product.java
    Then if it was successfull called, it would return our url for "confirmpurchase".html. If there was not
    enough inventory for the matching product, you would instead be directed to "purchaseerror".html*/
    @GetMapping("/purchaseProduct")
    public  String purchaseProduct(@RequestParam("productID") int theID, Model theModel){
        ProductService boughtProductService = context.getBean(ProductService.class);
        Product boughtProduct = boughtProductService.findById(theID);

        boolean buyConfirmed = boughtProduct.purchaseProduct();
        if(buyConfirmed == true) {
            boughtProductService.save(boughtProduct);
